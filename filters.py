...
class MemberCanRestrictFilter(BoundFilter):
    ...
    async def check(self, message: types.Message):
        ...
        # I don't know why, but telegram thinks,
        # if member is chat creator, he cant restrict member
        return (member.is_chat_creator() or
                member.can_restrict_members) == self.member_can_restrict
